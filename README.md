...
결론적으로 입문자들에게 조금 더 흥미롭고 도움이 되어 다른 유사 기업들과 차별점을 두려고 진행한 프로젝트라고 생각함.
...
- Water-Net의 문제점도 있었지만 지식과 학습이 부족하여 모델수정을 못함

- 이미지 전처리를 통해 모델의 성능을 높였다면 다음에는 공부를 통해 모델의 구조를 변경 해보고  

- 데이터 수집단계에서는 수중생물에 대한 지식이 없어서 카테고리 선정에서 시간을 많이 소비

- 이미지전처리 모델과 객체탐지 모델을 하나로 병합시키는 것을 해보고 싶음
